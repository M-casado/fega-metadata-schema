...
The ``schemas/entities`` directory contains the **modular JSON Schema files** that collectively define the **Federated EGA (FEGA) metadata model**. Most JSON schema files in here correspond to a specific metadata **entity type** (e.g., [``cohort/schema.json``](./cohort/schema.json)) in the FEGA model. Shared definitions live in [``../common/schema.json``](../common/schema.json), generic graph constraints live in [``../graph/schema.json``](../graph/schema.json), and third-party constraints live in [``../../standards/json-schema/``](../../standards/json-schema/). By organizing the model into separate schema modules, we achieve a flexible, composable design where each entity (e.g. a *Biomaterial*) can be understood and validated independently. Furthermore, compound groupings like RDF Graphs, which may include flattened information from multiple entities, can be validated through [``../graph/schema.json``](../graph/schema.json). This modular approach also reflects the real-world structure of submissions – for example, a study may involve multiple biomaterials, processes, and data files linked together.
...
Inside the [``standards``](../../standards/) directory, we maintain validation constraints (e.g., JSON Schema, SHACL Shapes) derived or directly taken from third-party standards that are relevant to the FEGA metadata model. These are sometimes directly referenced within the EGA model entities, or they are used for other layers of validation (e.g., HealthDCAT-AP validation through SHACL Shapes).
